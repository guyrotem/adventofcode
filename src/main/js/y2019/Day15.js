...
class BfsTraversal {
    constructor() {
        this.visited = [{x: 0, y: 0, type: -1, distance: 0}];
        this.longestSolutions = [{
            path: [],
            type: -1
        }];
    }
    ...
    extendSolution(path) {
        return [1,2,3,4]
            .map((nextStep) => {
                let newPath = [...path, nextStep];
                return {
                    newPath: newPath,
                    coordinates: pathCoordinates(newPath)
                }
            })
            .filter(result => {
                return this.visited.findIndex(function(v) {
                    return v.x === result.coordinates.x && v.y === result.coordinates.y;
                }) < 0;
            })
            .map(function (result) {
                let machineResult = startNewGame().runUntilMoreInput(result.newPath).output;
                return {
                    newPath: result.newPath,
                    coordinates: result.coordinates,
                    type: machineResult[machineResult.length - 1]
                };
            })
            .filter(result => result.type !== 0)
            .map(result => {
                this.visited.push({x: result.coordinates.x, y: result.coordinates.y, type: result.type, distance: result.newPath.length});
                return {
                    path: result.newPath,
                    type: result.type
                };
    ...
    traverseUntil(pred = () => false) {
        let prev;
        while (this.longestSolutions.length > 0 && this.longestSolutions.findIndex(s => pred(s)) < 0) {
            prev = this.longestSolutions;
            this.longestSolutions = [].concat.apply([], this.longestSolutions.map(s => this.extendSolution(s.path)));
        }
        return {
            previousIteration: prev,
            lastIteration: this.longestSolutions
        };
    }
}

// 1
const mazeTraversal1 = new BfsTraversal();
mazeTraversal1.traverseUntil(solution => solution.type === 2);
console.log(mazeTraversal1.longestSolutions.find(s => s.type === 2).path.length);

//  2
const bfs2 = new BfsTraversal();
let prevSolutions = bfs2.traverseUntil().previousIteration;

console.log('Number of long path(s)' + prevSolutions.length);
console.log('Longest path from start ' + prevSolutions[0].path.length);

function drawMaze(elements) {
    const maxWidth = elements.map(el => el.x).reduce(function (agg, cur) {
        return Math.max(agg, cur)
    }, 0);

    const minWidth = elements.map(el => el.x).reduce(function (agg, cur) {
        return Math.min(agg, cur)
    }, maxWidth);

    const maxHeight = elements.map(el => el.y).reduce(function (agg, cur) {
        return Math.max(agg, cur)
    }, 0);

    const minHeight = elements.map(el => el.y).reduce(function (agg, cur) {
        return Math.min(agg, cur)
    }, maxHeight);
    ...
    for (let i = minHeight; i <= maxHeight; i++) {
        ...
        for (let j = minWidth; j <= maxWidth; j++) {
            let el = elements.find(el => el.y === i && el.x === j);
            if (!!el && el.type === 2)
                row += '~@@~';
            else if (!!el && el.type === -1)
                row += '~XX~';
            else if (!!el)
                row += '|' + pad(el.distance);
            else
...
drawMaze(bfs2.visited);
//  oxygen is 244 from start. 514 is the element farthest away (244 => 240 => 514 ~~> 278)